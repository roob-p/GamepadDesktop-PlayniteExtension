...
- The default keys are:
  ...
  - Reachable positions of sections and panels *(`[Global]` section):*
    ...
    - `$sectionNpanelx`, `$sectionNpanely` → reachable points of the `Explorer's Tab` (if you are in the first zone) or the `Zoom control bar` (if you are in the second zone) with `$tabjump_back` and `$tabjump_next` *(`LT` and `RT` by default).*   
