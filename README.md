...
- When Playnite is not focused, `GamepadDesktop` still allows control of the mouse (movement, right click, and left click with both `$mouseleft` and `$tabjump_next`) for the duration specified by `$time`. This feature is intended to let you interact with other windows or serve as an "emergency" workaround if the OS steals focus. Please remember to click back on `Playnite` once you have finished interacting with the prompted clients or launchers. After that period, the script goes idle until Playnite is focused again.
