...
This period is meant to let you interact with game launchers or clients *(e.g., Steam, GOG)* to install games.  
Please don't forget to click on `Playnite` once you're done.    
This period also serves as an "emergency" fallback in case the OS steals focus (e.g., due to a software update).  
After that time, the script enters idle mode until `Playnite` is active again.  
